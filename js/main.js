$(document).ready(function() {
  ...
  function ValidMail(email) {
    ...
    !valid
      ...
      : (email.style.outline = "1px solid #ccc");
  ...
  function ValidPhone(phone) {
    ...
    !valid
      ...
      : (phone.style.outline = "1px solid #ccc");
  ...
  $("form").on("submit", function(e) {
    ...
    if (emailValid && telValid) {
      ...
      allInput.forEach(el => (el.style.outline = "1px solid #ccc"));
      var $form = $(this);
...
let faq = document.querySelector(".section_faq-menu");

function accordion(e) {
  let arrCheck = [...document.querySelectorAll(".section_faq-menu-item > p")];
  let primaryArr = [...document.querySelectorAll(".primary")];
  let secondaryArr = [...document.querySelectorAll(".secondary")];
  secondaryArr.forEach(el => (el.style.display = "none"));
  window.innerWidth >= 768
    ? primaryArr.forEach(el => (el.style.display = "block"))
    : null;
  arrCheck.forEach(el =>
    el.hasAttribute("id") ? el.removeAttribute("id") : null
  );
  e.target
    .closest(".section_faq-menu-item")
    .querySelector("p")
    .setAttribute("id", "active");
  e.target
    .closest(".section_faq-menu-item")
    .querySelector(".primary").style.display = "none";
  window.innerWidth >= 768
    ? (e.target
        .closest(".section_faq-menu-item")
        .querySelector(".secondary").style.display = "block")
    : null;
}
faq.addEventListener("click", accordion);
faq.addEventListener("touchstart", accordion);
faq.addEventListener("touchend", accordion);
faq.addEventListener("touchcancel", accordion);
faq.addEventListener("touchmove", accordion);